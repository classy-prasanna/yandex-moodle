...
export class CoreCoursesCoursePreviewPage implements OnDestroy {
    ...
    protected yandexReturnUrl: string;
    ...
    ionViewDidLoad(): void {
        ...
        this.yandexEnabled = this.course.enrollmentmethods && this.course.enrollmentmethods.indexOf('yandex') > -1;
        ...
        this.yandexReturnUrl = this.textUtils.concatenatePaths(currentSiteUrl, 'enrol/yandex/success.php');
    ...
    yandexEnrol(): void {
        ...
        const urlLoaded = (event): void => {
                if (event.url.indexOf(this.yandexReturnUrl) != -1) {
                    hasReturnedFromYandex = true;
                } else if (event.url.indexOf(this.courseUrl) != -1 && hasReturnedFromYandex) {
